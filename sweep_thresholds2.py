"""Extended sweep around the best region."""
...
print(f"{'FILTER_TH':>10} {'SUM_TH':>10} {'Correct':>7} {'Total':>5} {'Accuracy':>8}")
print("-" * 50)

best_acc = 0
best_params = None
for filter_th in [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.7]:
    for sum_th in [1.5, 1.8, 2.0, 2.2, 2.5, 3.0, 3.5, 4.0, 5.0]:
        results_dict, _ = process_with_thresholds(str_result_file, filter_th, sum_th, useBias=True)
        cons = consolidated_results(image_dir, results_dict, illegible_path, soccer_ball_list)
        c, t, a = evaluate(cons, gt_dict)
        if a > best_acc:
            best_acc = a
            best_params = (filter_th, sum_th, c)
        if a >= 87.0:
            print(f"{filter_th:>10.2f} {sum_th:>10.1f} {c:>7} {t:>5} {a:>7.2f}%")

print(f"\n{'='*50}")
print(f"BEST: filter_th={best_params[0]}, sum_th={best_params[1]}")
print(f"      Correct={best_params[2]}, Accuracy={best_acc:.2f}%")
