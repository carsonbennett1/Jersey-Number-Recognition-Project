...
└────────┬─────────────┘
         ▼
┌──────────────────────┐
│ 8. Consolidation     │  Combine per-image STR predictions into one number per tracklet
└────────┬─────────────┘
         ▼
    Final Jersey Number
    per Tracklet
...
The SoccerNet flow above matches the graphs below (view on GitHub, or with a Mermaid preview in VS Code / Cursor).

### Stage flow (SoccerNet)
...
flowchart TB
    ...
    subgraph PostProcess [Post-Processing]
        Consolidate[Weighted voting / consolidation]
...
**File:** `helpers.py` → `process_jersey_id_predictions()` (default in `main.py`), plus `process_jersey_id_predictions_bayesian()` / `process_jersey_id_predictions_raw()` for experiments
**Conda env:** `jersey`
...
2. For each tracklet, **weighted-sum voting** on hard labels using per-image confidences (product of character probs), optional digit bias, and thresholds (`FILTER_THRESHOLD`, `SUM_THRESHOLD`) to drop weak evidence or abstain
3. `helpers.py` also implements **Bayesian log-likelihood aggregation** and other variants for research / sweeps
...
| `number_classifier.py` | Alternative ResNet34-based jersey classifier (non-STR path) |
| `data.py` | SoccerNet dataset downloader / helpers |
...
### Cloned / external repos (on disk)

- **`str/parseq/`** — PARSeq STR training and inference
- **`pose/ViTPose/`** — ViTPose pose estimation
- **`reid/centroids-reid/`** — Centroid-ReID
- **`sam2/`** — SAM optimizer (legibility training)
...
### Intermediate data formats (quick reference)

**Input**

- Tracklets: `{tracklet_id}/image_*.jpg`
- Ground truth: JSON `{tracklet_id: jersey_number}`

**Intermediate**

- ReID features: `{tracklet_id}_features.npy`
- Legibility: `legible.json` / `illegible.json` (`{tracklet_id: [image_paths]}` etc., per pipeline outputs)
- Pose: `pose_results.json` with per-image keypoints
- Crops: `crops/imgs/{image_name}.jpg`

**Output**

- STR: `jersey_id_results.json` — `{image_key: {label, confidence, raw, logits}}`
...
8. **Consolidation** combines this prediction with 15 other frames from the same tracklet (default: weighted voting over confidences; optional Bayesian aggregation in `helpers`) — "23" wins with the highest aggregate score
